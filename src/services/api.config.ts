 ...
 * @returns true if token is expired, false if valid or no expiration stored (token remains valid until backend invalidates)
 */
export const isAccessTokenExpired = (): boolean => {
  const expiredAt = getAccessTokenExpiredAt();
  if (!expiredAt) {
    // No expiration stored (e.g. API didn't send expires_at) → treat as valid so refresh keeps user logged in
    return false;
  }
  // expiredAt is in seconds, Date.now() is in milliseconds
  const now = Math.floor(Date.now() / 1000);
